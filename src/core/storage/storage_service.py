...
from . import aws_cli_transfer
from .s3_transfer_common import STALE_DOWNLOAD_ABORT_MESSAGE
...
class StorageService:
    ...
    def _upload_path_with_boto3(
        ...
        *,
        ...
        for attempt in range(1, S3_TRANSFER_RETRY_ATTEMPTS + 1):
            try:
                logger.info(
                    "Uploading local file to S3 with boto3: key=%s source=%s attempt=%s/%s",
                    key,
                    source,
                    attempt,
                ...
                logger.info(
                    "Uploaded local file to S3 successfully with boto3: key=%s source=%s attempt=%s/%s",
                    key,
                    source,
                    attempt,
                    S3_TRANSFER_RETRY_ATTEMPTS,
                ...
                if attempt >= S3_TRANSFER_RETRY_ATTEMPTS:
                    logger.error(
                        "Failed to upload local file to S3 with boto3: key=%s source=%s attempt=%s/%s",
                        key,
                        source,
                        attempt,
                        S3_TRANSFER_RETRY_ATTEMPTS,
                ...
                logger.warning(
                    "Retrying S3 upload with boto3 after transient error: key=%s source=%s attempt=%s/%s backoff=%ss error=%s",
                    key,
                    source,
                    attempt,
                    S3_TRANSFER_RETRY_ATTEMPTS,
        ...
        *,
        ...
        for attempt in range(1, S3_DOWNLOAD_RETRY_ATTEMPTS + 1):
            ...
            try:
                ...
                logger.info(
                    "Downloading S3 object to local path with boto3: key=%s destination=%s attempt=%s/%s",
                    key,
                    destination,
                    attempt,
                    ...
                    while True:
                        ...
                        if now - last_progress_log_at >= S3_PROGRESS_LOG_INTERVAL_SECONDS:
                            if content_length > 0:
                                ...
                                logger.info(
                                    "S3 download progress with boto3: key=%s destination=%s attempt=%s/%s downloaded=%s/%s bytes (%.1f%%)",
                                    key,
                                    destination,
                                    attempt,
                                    S3_DOWNLOAD_RETRY_ATTEMPTS,
                                ...
                                logger.info(
                                    "S3 download progress with boto3: key=%s destination=%s attempt=%s/%s downloaded=%s bytes",
                                    key,
                                    destination,
                                    attempt,
                                    S3_DOWNLOAD_RETRY_ATTEMPTS,
                ...
                logger.info(
                    "Downloaded S3 object successfully with boto3: key=%s destination=%s attempt=%s/%s bytes=%s",
                    key,
                    destination,
                    attempt,
                    S3_DOWNLOAD_RETRY_ATTEMPTS,
                ...
                if STALE_DOWNLOAD_ABORT_MESSAGE in str(exc):
                    logger.info(
                        "Stopped S3 download with boto3 because merge request became stale: key=%s destination=%s attempt=%s/%s",
                        key,
                        destination,
                        attempt,
                        S3_DOWNLOAD_RETRY_ATTEMPTS,
                ...
                if is_last_attempt or not retryable:
                    logger.error(
                        "Failed to download S3 object with boto3: key=%s destination=%s attempt=%s/%s",
                        key,
                        destination,
                        attempt,
                        S3_DOWNLOAD_RETRY_ATTEMPTS,
                ...
                logger.warning(
                    "Retrying S3 download with boto3 after transient error: key=%s attempt=%s/%s backoff=%ss error=%s",
                    key,
                    attempt,
                    S3_DOWNLOAD_RETRY_ATTEMPTS,
                    backoff_seconds,
    ...
    def upload_path(
        ...
        *,
        ...
        if self.is_s3:
            ...
            logger.info(
                "Selected S3 upload engine: engine=%s key=%s source=%s",
                upload_engine,
                key,
                source,
            )
            if upload_engine in EXTERNAL_TRANSFER_ENGINES:
                try:
                    ...
                    logger.warning(
                        "Falling back to boto3 upload after external engine failure: engine=%s key=%s source=%s error=%s",
                        upload_engine,
                        key,
                        source,
                        exc,
    ...
    def upload_fileobj(
        ...
        *,
        ...
        if self.is_s3:
            logger.info(
                "Using boto3-only S3 upload for file object: key=%s file_name=%s",
                key,
                file_name,
            )
            extra_args = {}
    ...
    def download_to_path(
        ...
        *,
        ...
        if self.is_s3:
            ...
            logger.info(
                "Selected S3 download engine: engine=%s key=%s destination=%s",
                selected_engine,
                key,
                destination,
            )
            if selected_engine in EXTERNAL_TRANSFER_ENGINES:
                try:
                    ...
                    if STALE_DOWNLOAD_ABORT_MESSAGE in str(exc):
                        logger.info(
                            "Stopped S3 download because merge request became stale before boto3 fallback: key=%s destination=%s",
                            key,
                            destination,
                        )
                        raise
                    logger.warning(
                        "Falling back to boto3 download after external engine failure: engine=%s key=%s destination=%s error=%s",
                        selected_engine,
                        key,
                        destination,
                        exc,
    ...
    def complete_multipart_upload(
        ...
        *,
        ...
        logger.info(
            "Completing S3 multipart upload: key=%s parts=%s",
            key,
        ...
        try:
            ...
            logger.error(
                "Failed to complete S3 multipart upload: key=%s parts=%s",
                key,
                len(normalized_parts),
                exc_info=True,
            )
            raise
        logger.info(
            "Completed S3 multipart upload in %.2fs: key=%s parts=%s",
